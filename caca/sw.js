/* Caca SW — cache-first (les pages en cache, les données via réseau) */
const CACHE = 'caca-v4';
...
self.addEventListener('fetch',e=>{
  ...
  if(e.request.url.includes('firestore')||e.request.url.includes('firebase')||e.request.url.includes('firebasestorage')||e.request.url.includes('googleapis')||e.request.url.includes('gstatic'))return; // données toujours en direct
